...
from ..cnab.registro_online.schemas import SchemaBoletoCaixa, SchemaBoletoBB
...
@router.post("/validar-json-caixa")
async def validar_json_caixa(payload: dict):
    """Valida a estrutura de um JSON de Registro Online da Caixa Econômica."""
    try:
        SchemaBoletoCaixa(**payload)
        ...
        for erro in e.errors():
            ...
            erros_formatados.append({
                "mensagem": f"Erro estrutural no JSON: {erro['msg']}",
                "campo": campo,
                "valor_encontrado": str(erro.get("input", "N/A")),
                "sugestao_rm": "Verifique o mapeamento deste campo no cadastro da filial ou do cliente no RM."
            })
        
        registrar_uso("registro_online", "validar-json-caixa", {"valido": False, "total_erros": len(erros_formatados)})
        return {"valido": False, "erros": erros_formatados}


@router.post("/validar-json-bb")
async def validar_json_bb(payload: dict):
    """Valida a estrutura de um JSON de Registro Online do Banco do Brasil."""
    try:
        SchemaBoletoBB(**payload)
        return {"valido": True, "erros": []}
    except ValidationError as e:
        erros_formatados = []
        for erro in e.errors():
        ...
        registrar_uso("registro_online", "validar-json-bb", {"valido": False, "total_erros": len(erros_formatados)})
        return {"valido": False, "erros": erros_formatados}    